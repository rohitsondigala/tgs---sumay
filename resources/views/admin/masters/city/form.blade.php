...
@section('content')
    <div class="col-md-8 mt-3 offset-2">
        <div class="card card-default">
            ...
            <div class="card-body">
                @if($city->id)
                    {!! Form::model($city,array('url'=>route($route.'.update',$city->id),'method'=>'PUT')) !!}
                @else
                    {!! Form::model($city,array('url'=>route($route.'.store'),'method'=>'post')) !!}
                @endif
                <div class="row">
                    ...
                    <div class="form-group col-md-12 mb-4">
                        {!! Form::select('state_id',$states,null,['class'=>'form-control select2']) !!}
                        @if($errors->has('state_id'))
                            <span class="text text-danger">{{$errors->first('state_id')}}</span>
                        @endif
                    </div>
